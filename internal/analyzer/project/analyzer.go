...
func (Analyzer) AutoAnalysis(gitlabClient *gitlab.Client, options *types.Options, config *viper.Viper, output *types.Output) {
	...
	AllProjects, projectsService, approvalService, projectMembersService, protectedBranchesService, projectAccessTokensService, projectValidateService, runnersService := ListAllProjects(gitlabClient)
	output.Projects.Projects = make([]types.Project, 0, len(AllProjects))

	var wg sync.WaitGroup

	for _, single_project := range AllProjects {
		if single_project.DefaultBranch == "" {
			continue
		}

		wg.Add(1)
		go func(single_project *gitlab.Project) {
			defer wg.Done()
			...
			// 获取CICD，检查 stage 是否有2或2个以上的阶段来判断合规
			...
			if cicdContent.Valid {
				...
				if o_single_project.CICDYamlStageNum >= 2 {
			...
			// 获取项目 runner 检查是否被设置为共享
			runners, _, err := runnersService.ListProjectRunners(single_project.ID, nil, nil)
			if err != nil {
				fmt.Println(err)
			}
			var tmp_des string
			runner_list := make([]string, 0, len(runners))
			for _, v := range runners {
				if v.IsShared {
					tmp_des = fmt.Sprintf("共享runner: %s, %s", strconv.Itoa(v.ID), v.Description)
					runner_list = append(runner_list, tmp_des)
				}
			}
			o_single_project.RunnersDescription = strings.Join(runner_list, "\n")
